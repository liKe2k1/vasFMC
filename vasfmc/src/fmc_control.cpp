...
#ifdef HAVE_PLIB
    else if (m_main_config->getValue(CFG_FS_ACCESS_TYPE) == FS_ACCESS_TYPE_FGFS)
    {
        Logger::log("Switching to flightgear access");
        m_fs_access = new FSAccessFgFs(m_config_widget_provider, CFG_FGFS_FILENAME, m_flightstatus);
    }
#endif /* HAVE_PLIB */
#ifdef Q_OS_WIN32
    // Disable instatiation of an FSAccessMsFs-Object if not in win32-environment
    else
    {
        Logger::log("Switching to MSFS access");
        m_fs_access = new FSAccessMsFs(m_config_widget_provider, CFG_MSFS_FILENAME, m_flightstatus);
    }
#else
    else
    {
        Logger::log("Could not resolve sim-type, probably MSFS on a non-windows platform. Forcing XPLANE access.");
        switchToXPlane();
    }
#endif
...
void FMCControl::switchToMSFS()
{
// Make sure that the slot can not write access=msfs to the .cfg-file if not in win32-environment
#ifdef Q_OS_WIN32
    m_main_config->setValue(CFG_FS_ACCESS_TYPE, FS_ACCESS_TYPE_MSFS);
#else
    Logger::log("Switching to MSFS on a non-windows platform is not supported.");
#endif
    setupFsAccess();
}
...
{
    m_main_config->setValue(CFG_FS_ACCESS_TYPE, FS_ACCESS_TYPE_FGFS);
    setupFsAccess();
}
#endif /* HAVE_PLIB */
