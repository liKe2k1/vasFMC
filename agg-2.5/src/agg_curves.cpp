...
// MB 25.1.09 For error message output
#include <cstdio>
...
namespace agg
{
    ...
    // Workaround to prevent (almost) infinite recursion in some cases, still
    // have to find the reason for the infinite recursion MB 24.1.09
    // enum curve_recursion_limit_e { curve_recursion_limit = 32 };
    enum curve_recursion_limit_e { curve_recursion_limit = 8 };

    curve4_div::error_callback curve4_div::s_error_callback=0;
                                      ...
                                      unsigned level)
    {
        ...
        if(level > curve_recursion_limit) 
        {
            // MB 25.1.09 Output error message if we reach the recursion limit
            if(s_error_callback!=0 && first_error)
            {
                first_error=false;
                snprintf(error_msg, 512, "Reached recursion limit in "
                    "curve4_div::recursive_bezier\nArguments:\n"
                    "x1 %.15g\ny1 %.15g\nx2 %.15g\ny2 %.15g\n"
                    "x3 %.15g\ny3 %.15g\nx4 %.15g\ny4 %.15g\n",
                    x1, y1, x2, y2, x3, y3, x4, y4);
                s_error_callback(error_msg);
            }
            return;
        }
