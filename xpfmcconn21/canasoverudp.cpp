...
/*extern DataContainer<double> doubleData;
extern DataContainer<float> floatData;
extern DataContainer<int> intData;
extern DataContainer<bool> boolData;
extern DataContainer<std::vector<float> > floatvectorData;
extern DataContainer<std::vector<int> > intvectorData;
extern DataContainer<std::string> stringData;
extern std::vector<LogicHandler*> Handlers;*/
...
uint getFloatVectorFromCan(can_t can, floatvec& outFloatVector)
{
    if (can.dlc == 5 && can.msg.aero.dataType == AS_USHORT) {
        outFloatVector.resize(can.msg.aero.data.uShort[0]);
        return outFloatVector.size;
    } else {
        MYASSERT(can.msg.aero.dataType = AS_FLOAT);
        MYASSERT(can.msg.aero.serviceCode < outFloatVector.size);
        outFloatVector.value[can.msg.aero.serviceCode] = can.msg.aero.data.flt;
...
bool CanASOverUDP::configure(Settings* cfg, const std::string& cfg_section, const std::string& cfg_port_from_sim,
                        const std::string& cfg_host, const std::string& cfg_port_to_sim)
{
    // get writehost and writeport from config file and configure UDP Socket to write
    writeSock->configure(cfg->GetValue(cfg_section.c_str(),cfg_host.c_str()),
                         cfg->GetLongValue(cfg_section.c_str(),cfg_port_from_sim.c_str()));

    readSock->configure(cfg->GetValue(cfg_section.c_str(),cfg_host.c_str()),
                        cfg->GetLongValue(cfg_section.c_str(),cfg_port_to_sim.c_str()));
    configured = true;
    return true;
}
                            ...
                            int ticks, double secs, unsigned int maxDataItems)
{
    ...
    //printf("Got %i items queued before update ",dynamic_cast<Casprotocol*>(casprotocol)->lengthOfQueue());
    doubleData.writeOutdated(casprotocol, ticks, secs);
    floatData.writeOutdated(casprotocol, ticks, secs);
    intData.writeOutdated(casprotocol, ticks, secs);
    boolData.writeOutdated(casprotocol, ticks, secs);
    ...
    //printf("Got %i items queued now after update ",dynamic_cast<Casprotocol*>(casprotocol)->lengthOfQueue());
...
void CanASOverUDP::flush(unsigned int maxDataItems)
{
    ...
    //printf("%i items remaining in queue now after send \n",dynamic_cast<Casprotocol*>(casprotocol)->lengthOfQueue());
}
...
{
    /*int ticks = XPLMGetElapsedTime();
    float secs =XPLMGetCycleNumber();
    processInput(intData, floatData, doubleData, boolData, floatvectorData, intvectorData, stringData, Handlers);
    processOutput(intData, floatData, doubleData, boolData, floatvectorData, intvectorData, stringData, Handlers, ticks, secs);
    */
    return true;
}
...
template <>
void CanASOverUDP::appendCanMsg<std::vector<float> >(std::queue<std::pair<unsigned char, std::vector<float> > > dataQueue)
{
    while (!dataQueue.empty())
    {
        pair<unsigned char, std::vector<float> > item = dataQueue.front();
        uint size = item.second.size();
        canAS_t message;
        can_t can;
        can.id = uint32_t(item.first);
        can.id_is_29 = PLUGIN_USES_ID29;
        message.nodeId = PLUGIN_NODE_ID;
        message.messageCode = messageCode;
        message.serviceCode = 0;
        message.dataType = AS_USHORT;
        message.data.uShort[0] = size;
        can.dlc = 5;
        ...
        for (uint i = 0 ; i < size ; i++ )
        {
            ...
            message.data.flt = item.second[i];
