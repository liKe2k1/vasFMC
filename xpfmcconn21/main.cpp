...
void setupDefaultConfig(Settings *cfg)
{
    MYASSERT(cfg);

    cfg->SetLongValue(CFG_SECTION, CFG_PORT_TO_SIM, CFG_PORT_TO_SIM_DEFAULT);
    cfg->SetValue(CFG_SECTION, CFG_HOSTADDRESS, CFG_HOSTADDRESS_DEFAULT);
    cfg->SetLongValue(CFG_SECTION, CFG_PORT_FROM_SIM, CFG_PORT_FROM_SIM_DEFAULT);
    cfg->SetValue(CFG_SECTION, CFG_FAST_NETWORK, CFG_FAST_NETWORK_DEFAULT);
    cfg->SetValue(CFG_SECTION, CFG_POLLING_POLICY, CFG_POLLING_POLICY_DEFAULT);
}
...
void setup()
{
    ...
    //Config *m_cfg = new Config(CFG_FILENAME, "=");
    Settings *cfg = new Settings(CFG_FILENAME);
    MYASSERT(cfg);
    ...
    myCommunicator->configure(cfg, CFG_SECTION, CFG_PORT_FROM_SIM, CFG_HOSTADDRESS, CFG_PORT_TO_SIM);
    if (strcmp( cfg->GetValue(CFG_SECTION, CFG_POLLING_POLICY), "time") == 0)
    {
        m_logfile << "Policy is time, looking for fast network" << std::endl;
        if (strcmp(cfg->GetValue(CFG_SECTION, CFG_FAST_NETWORK), "true") == 0)
        {
            m_logfile << "Fast network is true, so set fast time policy to 0.17 seconds" << std::endl;
            returnTime = 0.17f;
        } else if (strcmp(cfg->GetValue(CFG_SECTION, CFG_FAST_NETWORK), "false") == 0)
        {
            m_logfile << "Fast network is false, so set slow time policy to 0.33 seconds" << std::endl;
            returnTime = 0.33f;
        } else {
            m_logfile << "ERROR: Unrecognized network time policy, defaulting to fast. Please correct your config." << std::endl;
            returnTime = 0.17f;
        }
    } else if (strcmp( cfg->GetValue(CFG_SECTION, CFG_POLLING_POLICY), "frames") == 0)
    {
        m_logfile << "Policy is frames, looking for fast network" << std::endl;
        if (strcmp(cfg->GetValue(CFG_SECTION, CFG_FAST_NETWORK), "true") == 0)
        {
            m_logfile << "Fast network is true, so set fast frames policy to 5 frames" << std::endl;
            returnTime = -5.0f;
        } else if (strcmp(cfg->GetValue(CFG_SECTION, CFG_FAST_NETWORK), "false") == 0)
        {
            m_logfile << "Fast network is false, so set slow time policy to 9 frames" << std::endl;
            returnTime = -9.0f;
        } else {
            m_logfile << "ERROR: Unrecognized network frame policy, defaulting to fast. Please correct your config." << std::endl;
            returnTime = -5.0f;
        }
    } else
    {
        m_logfile << "ERROR: Unrecognized polling policy, defaulting to fast time. Please correct your config." << std::endl;
        returnTime = 0.17f;
    }

    if (!cfg->saveToFile())
        m_logfile << "could not save config" << std::endl;
    else
        m_logfile << "saved config" << std::endl;
    delete cfg;
    cfg = 0;
