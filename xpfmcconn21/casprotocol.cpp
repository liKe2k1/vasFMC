...
void Casprotocol::protocolWrite(uint32_t id, std::string data, uint8_t message_code)
{
    ...
    if (strlen > 0)
    {
        if (strlen > 5)
        {
            std::cerr << "Error, unable to send strings longer than 5 chars with this CANaerospace distribution!" << std::endl;
            MYASSERT(false);
        }
...
unsigned int Casprotocol::writeMax(unsigned int max)
{
    ...
    {
        std::cerr << "The sendqueue grows too large, skipping old items now." << std::endl;
        while (!m_sendqueue.empty())
            m_sendqueue.pop();
        grown_in_a_row = 0;
    }
