...
bool RadioNav::processState()
{
        ...
        m_navReceiverStates->hasChanged() || true )
    {
        ...
        ///////
            ...
            //std::cout <<"navtpye nav1:" << navmode.at(0) << std::endl;
...
navaid RadioNav::locateNavAid(int freq, float lat, float lon, XPLMNavType type, bool has_dme)
{
    ...
    {
        case (xplm_Nav_NDB):
            //out= new Ndb(outID,outName,outLatitude,outLongitude,outFrequency,50,int(round(outHeight)),"");
            break;
        case (xplm_Nav_VOR):
            //out = new Vor(outID,outName,outLatitude,outLongitude,outFrequency,has_dme,50,int(round(outHeight)),"");
            out.HasDME = has_dme;
            out.HasLoc = false;
            break;
        case (xplm_Nav_ILS):
        ...
        case (xplm_Nav_GlideSlope):
            //out = new Ils(outID,outName,outLatitude,outLongitude,outFrequency,has_dme,50,int(round(outHeight)),"",int(round(outHeading)));
            out.crs = Navcalc::round(outHeading);
            out.HasDME = has_dme;
            out.HasLoc = true;
            break;
            //default: out = new Waypoint(outID,outName,outLatitude,outLongitude);break;
    }    return out;
}
