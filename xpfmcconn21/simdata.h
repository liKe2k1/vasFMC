...
m_no_of_items(no_of_items)
{
    if (!lookUp())
    {
        std::cerr << "DataRef not found for " << m_name << std::endl;
        exit(1);
    }
    if(!checkDataType())
    {
        std::cerr << "Wrong Datatype for " << m_name << std::endl;
        exit(1);
    }
    if(!checkWriteabilityIsValid())
    {
        std::cerr << m_name << " was declared to be writeable, but XPlane says it is read-only" << std::endl;
        exit(1);
    }
    m_hasChanged = true;
}
...
bool SimData<T>::poll()
{
    std::cerr << "Poll called with incorrect type for " << m_name << std::endl;
    return false;
}
...
{
    std::cerr << "Error: datatype of SimData<T>-instance unknown" << std::endl;
    return false;
}
template <>
...
{
    if (isnan(data)) {
        std::cerr << "update value returned nan in " << this->name() << std::endl;
        this->m_data = 0;
        //exit(1);
    }
...
T SimData<T>::data()
{
    if (isnan(this->m_data)){
        std::cerr << "data() returned nan in " << this->name() << std::endl;
        this->m_data = 0;
        //exit(1);
    }
