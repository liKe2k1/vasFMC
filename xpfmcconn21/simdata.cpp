...
{
    if (m_pDataRef == 0) {
        std::cerr << "In polling no DataRef for " << this->name() << std::endl;
        exit(1);
    }
    double temp = (double)XPLMGetDatad(m_pDataRef);
    if (isnan(temp)){
        std::cerr << "In polling nan from XPlane for " << this->name() << std::endl;
        updateValue(0);
        //exit(1);
    }
    if(m_readWrite == RWType::WriteOnly) return false;
...
bool SimData<int>::set(int data)
{
    ...
    m_hasChanged = true;
    if (m_scale != 1)
        setValue(int(round((data-m_offset)/m_scale)));
    else
        setValue(data);
    return m_hasChanged;
}
...
bool SimData<std::vector<float> >::set(std::vector<float> data)
{
    ...
    {
        std::cerr << "Requested set for vector "<< this->name() << " too long, cropping from "
...
bool SimData<std::vector<int> >::set(std::vector<int> data)
{
    ...
    if (m_scale != 1) {
        for( uint i=0 ; i<data.size() ; i++ )
        {
            data[i] = int(round((data[i]-m_offset)/m_scale));
        }
    }
    setValue(data);
    return m_hasChanged;
...
int SimData<int>::data()
{
    if (m_scale == 1 && m_offset == 0)
        return m_data;
    return int(round(m_data * m_scale + m_offset));
}
...
std::vector<int> SimData<std::vector<int> >::data()
{
    ...
    returnvector.resize(m_data.size());
    for (uint i=0 ; i<m_data.size() ; i++)
        returnvector[i]=int(round(m_data[i]*m_scale + m_offset));
