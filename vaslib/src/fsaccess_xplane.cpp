...
/*template <typename T>
bool FSAccessXPlane::sendValue(int id, T value)
{
    QByteArray sendbuffer;
    QDataStream ds(&sendbuffer, QIODevice::WriteOnly);
    ds << (unsigned char)id << (T)value;
    long sent_bytes = m_write_socketdevice->writeDatagram(sendbuffer.data(), sendbuffer.count(),m_write_hostaddress, m_writeport);
...
template<>
bool FSAccessXPlane::sendValue(int id, std::vector<float> value)
{
    QByteArray sendbuffer;
    QDataStream ds(&sendbuffer, QIODevice::WriteOnly);
    ds << (unsigned char)id << (uint)value.size();
...
void FSAccessXPlane::slotSocketRead()
{
    ...
    while(m_read_socketdevice->hasPendingDatagrams())
    {
        ...
        /* For network debugging only:
        QString reason;
        switch (canmsg.reason)
        {
            case 0 : reason = QString("Unspecified / Forced Write / Logic"); break;
            case 1 : reason = QString("Has Changed"); break;
            case 2 : reason = QString("Timed Out"); break;
            case 3 : reason = QString("Both Timed Out and changed"); break;
        }
        QString prio;
        switch (canmsg.prio)
        {
            case 0xc0: prio = QString("constant"); break;
            case 0: prio = QString("High / Logic"); break;
            case 0x40: prio = QString("Middle"); break;
            case 0x80: prio = QString("Low"); break;
        }
        Logger::log(QString("Received no %1, with prio %2, reason %3").arg(canmsg.this_send).arg(prio).arg(reason));
        */
        ...
        static int year = QDate::currentDate().year();
            ...
            case ZTIME: f = getFloatFromCan(canmsg); m_flightstatus->fs_utc_time = QTime().addSecs(int(round(f))); break;
            case ZDATE: i = getIntFromCan(canmsg); m_flightstatus->fs_utc_date = QDate(year,1,1).addDays(i); break;
            case V_S0: f = getFloatFromCan(canmsg); m_flightstatus->speed_vs0_kts = uint(round(f)); break;
            case V_S: f = getFloatFromCan(canmsg); m_flightstatus->speed_vs1_kts = uint(round(f));
                // green dot speed = 1.35 * stall speed in clean config
                m_flightstatus->speed_min_drag_kts = uint(round(f*1.35));
                break;
            ...
            case TOTWT:  f = getFloatFromCan(canmsg); m_flightstatus->total_weight_kg = uint(round(f)); break;
            case FUELCAP: f = getFloatFromCan(canmsg); m_flightstatus->total_fuel_capacity_kg = f; break;
            case FUELWT: f = getFloatFromCan(canmsg); m_flightstatus->zero_fuel_weight_kg = m_flightstatus->total_weight_kg - uint(round(f)); break;
